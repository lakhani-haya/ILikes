...
import { getMovieDetails } from '../providers/moviesOmdb';
...
export default function MovieDetailPage() {
  ...
  const libraryItem = useMemo(
    () => items.find(item => item.type === 'movie' && item.externalId === id),
    [id, items]
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error(err);
        if (mounted) setError('Unable to load movie details.');
      } finally {
  ...
  const handleAdd = () => {
    if (!details) return;
    addFromSearch('movie', toSearchResult(details));
  };
  ...
  const meta = useMemo(() => {
    ...
    if (details?.creator) entries.push({ label: 'Director', value: details.creator });
    if (details?.year) entries.push({ label: 'Year', value: details.year });
    if (details?.runtime) entries.push({ label: 'Runtime', value: details.runtime });
    return entries;
  }, [details]);
