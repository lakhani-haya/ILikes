...
import { getMusicDetails } from '../providers/musicItunes';
...
export default function MusicDetailPage() {
  ...
  const libraryItem = useMemo(
    () => items.find(item => item.type === 'music' && item.externalId === id),
    [id, items]
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (mounted) {
          if (!result) {
            setError('Unable to load music details.');
          } else {
            setDetails(result);
          }
        }
        ...
        console.error(err);
        if (mounted) setError('Unable to load music details.');
      } finally {
  ...
  const handleAdd = () => {
    if (!details) return;
    addFromSearch('music', toSearchResult(details));
  };
  ...
  const meta = useMemo(() => {
    ...
    if (details?.creator) entries.push({ label: 'Artist', value: details.creator });
    if (details?.year) entries.push({ label: 'Year', value: details.year });
    if (details?.runtime) entries.push({ label: 'Length', value: details.runtime });
    return entries;
  }, [details]);
  ...
  if (error || !details) {
    return (
      <section className="max-w-5xl mx-auto px-6 py-10">
        <p className="text-red-600">{error || 'Music not found.'}</p>
      </section>
    );
  }
